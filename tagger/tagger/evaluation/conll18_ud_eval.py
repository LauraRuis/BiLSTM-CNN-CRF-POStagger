...
def evaluate_18(gold_ud, system_ud):
    class Score:
...
def load_conllu_file(path):
    ...
    return load_conllu_18(_file)

def evaluate_wrapper(args):
    # Load CoNLL-U files
    gold_ud = load_conllu_file(args.gold_file)
    system_ud = load_conllu_file(args.system_file)
    return evaluate_18(gold_ud, system_ud)
...
class TestAlignment(unittest.TestCase):
    ...
    def _load_words(words):
        ...
        return load_conllu_18((io.StringIO if sys.version_info >= (3, 0) else io.BytesIO)("\n".join(lines+["\n"])))

    def _test_exception(self, gold, system):
        self.assertRaises(UDError, evaluate, self._load_words(gold), self._load_words(system))

    def _test_ok(self, gold, system, correct):
        metrics = evaluate_18(self._load_words(gold), self._load_words(system))
