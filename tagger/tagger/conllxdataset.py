...
from tagger.utils import PAD_TOKEN, UNK_TOKEN, ROOT_TOKEN
...
def get_data_fields():
  ...
  nesting_field = Field(tokenize=list, pad_token=PAD_TOKEN, batch_first=True,
                        init_token=ROOT_TOKEN, eos_token=None, preprocessing=lambda xs: [x.lower() for x in xs])
...
class ConllXDataset(Dataset):
  ...
  def __init__(self, path, fields, **kwargs):
    ...
    with io.open(os.path.expanduser(path), encoding="utf8") as f:
      examples = [Example.fromdict(d, fields) for d in conllx_reader(f)]
